...
academyDialog.add('/findCourse' , [
	function(session,args) {
		console.log("Skills Length :"+ args.entities.length);
                ...
                console.log('args.entities.length',args.entities.length);
		var course = [];
		// Don't work for wrong spellings
		for(var i=0; i < args.entities.length; i++) {
                        var course = args.entities[i].entity;
                        try{
...
//                      builder.Prompts.text(session,"Total Duration for learning "+session.userData.course +" is "+"**"+duration+"**"+" days");
                        ...
                        console.log("session.userData.duration"+session.userData.duration);
...
//			session.send(prompts.recommend_skills+"  \n"+"**"+prereq+"**" +" are the pre-requisites for learning "+session.userData.course+"."+"  \n"+" And the duration for the course is "+session.userData.duration+ " days.");
			...
			console.log("session.userData.duration in function "+session.userData.duration);
			var releventPrereq = "";
			var skillMatch = "";
			if(prereq.length==0){
				if(session.userData.duration=='null'){
					session.userData.duration = 0;
					builder.Prompts.text(session,prompts.recommend_skills+"  \n  \n "+"* "+" There are no pre-requisites for learning "+session.userData.course+"."+"  \n   \n "+"* The duration for the course is ** "+session.userData.duration+ "** days.");
				}
				else{
				builder.Prompts.text(session,prompts.recommend_skills+"  \n  \n "+"* "+" There are no pre-requisites for learning "+session.userData.course+"."+"  \n   \n "+"* The duration for the course is ** "+session.userData.duration+ "** days.");
			}
...
academyDialog.add('/userCheck', [
	function(session) {
		try
		{
	//	console.log('Session.userData.name' +session.UserData.name);
		console.log('utilities.isEmpty(session.userData.name)' + utilities.isEmpty(session.userData.name));
		} catch (err)
		{
